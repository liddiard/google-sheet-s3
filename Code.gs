...
function onOpen() { 
  createMenu();
  // prompt the user via email to reauthorize the add-on if necessary
  reauthorize();
}

// send user an email to reauthorize if the add-on requires new permissions
// https://developers.google.com/apps-script/guides/triggers/installable#authorization
function reauthorize() {
  var addonTitle = 'Publish sheet to S3';
  ...
  // else authorization has been granted, so do nothing.
}
